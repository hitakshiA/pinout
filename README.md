...
**A prepaid meter for anything that runs — settled over [x402](https://x402.org) on [Hedera](https://hedera.com).**
...
mid-flight without dropping the connection · unused credits are refunded on-chain ·
and **anyone can recompute the bill from the public ledger**.

Two things run on this meter: **token streams**, and **real machines rented by the second**.
...
- [Pinout Compute — rent a machine by the second](#pinout-compute--rent-a-machine-by-the-second)
- [Quickstart](#quickstart)
- [Watch it catch a cheating seller](#watch-it-catch-a-cheating-seller)
- [API](#api)
...
sequenceDiagram
    ...
    C->>S: POST /compute/cpu-4  (or /session)
    ...
    loop each unit consumed
        S-->>C: SSE event · 1 credit burned
    end
    S->>H: burn checkpoint → plain HCS topic
    Note over C,S: balance low → top-up 402 cycle<br/>work never stops
...
A session can only ever be credited with the amount that actually moved to the seller
in the transaction the buyer signed. This is structural: three separate money bugs in
this codebase were the same shape — a flat-priced 402, a credit read from a lane table,
and an on-chain refund of the difference — and deriving credit from the transfer makes
that class impossible rather than fixing it three times.

The refund is **not** gated behind the anchor. Gating it meant an anchor failure
stranded a buyer's balance. The anchor is queued and swept instead, and the verifier
reports `PENDING_ANCHOR` (exit 3) rather than accusing an honest seller during the
...
flowchart LR
    subgraph T1["Tier 1 · burn ledger"]
        A["plain HCS topic<br/>~$0.0008/write<br/>every N units"]
    end
...
Anchors are **batched by default** — one anchor covers many sessions. Per-session
...
## Pinout Compute — rent a machine by the second

The same verified meter, pointed at real hardware. One tick = one second held.
Providers: **Daytona** (CPU) and **Modal** (GPU).
...
from the *session*, never from the request — so a caller cannot pay `cpu-small` prices
and ask for a GPU.
...
**One-shot** is cheapest when the work is a single known script: submit it, it runs, the
machine is handed back the moment it exits, and you are billed only for the seconds it
actually ran.
...
await m.upload("/work/input.csv", buf);
const r = await m.exec("import pandas as pd; ...");   // state persists
const out = await m.download("/work/result.parquet"); // sha256-checked in transit
await m.release();                                    // unused seconds refunded
```

### Running out mid-job

Credits exhausting does **not** kill the machine. It pauses: the machine is held, the
meter stops, and `SessionWaiting` frames report the grace window draining. Top up and
the *same process* resumes from exactly where it stopped.
...
124 s, was billed **0** for 10 s of pause, resumed at **step 120 — the step it stopped
...
Testnet HBAR is free from a faucet, so **the payment gate provides no economic friction
on testnet**. Payment alone cannot be what stands between a stranger and a real GPU
bill. Admission is decided *before* the 402 is issued — never quote a price for work you
will not do:

- concurrency caps (sessions, and GPU sessions separately)
- per-lane wall-clock ceilings
- provider budget ceilings, charged in seconds **held** rather than seconds billed,
  with a GPU reserve held back
- **seller solvency** — new sessions are refused when the seller could not afford a
  settlement anchor for every open session, rather than risk stranding a prepaid balance
...
| `POST` | `/session/<id>/close` | 🔑 | Refund → anchor → signed receipt |
...
billed by the second. They refuse with `402` at zero credits, or a paused session would
be free compute.
...
Every agent surface is gated on confirmed on-chain payment. **No tool returns data
...
limits, HCS audit hooks and human-in-the-loop confirmation apply automatically, which is
exactly what you want on a payments plugin.
...
> independent agent worked out the protocol from the HTTP responses alone and paid.
> Its balance moved by exactly the purchase amount and not one tinybar more. A second
> agent, given no hints, rented a machine, uploaded a dataset, aggregated it, read that
> result back **in a separate step**, downloaded the verdict and released — 86 seconds
> billed, 34 refunded, summing exactly to the 120 bought.
...
received units. No API key, nothing to trust.
...
flowchart TD
    ...
    C3 --> C4{4 · burn count = units received?}
...
Under `--cheat`, checks 1–3 still pass. The ledger genuinely is immutable and internally
consistent — that is what HCS guarantees, and all it guarantees. Only comparing it
against the buyer's own record exposes fabrication. A seller that fabricates events *and*
whose client never notices is outside what this proves.
...
- **Unit economics do not close at demo scale.** A settlement anchor costs ~0.73 HBAR;
  a `cpu-small` session sells for 0.036 HBAR. Batching amortises it — break-even is
  roughly 2,450 billed seconds per anchor — but the seller subsidises small and
  abandoned sessions today. The numbers are in
  [`compute/PLAN.md` §10](./compute/PLAN.md), not hidden.
- **Cold start is absorbed by the seller.** The buyer's meter starts after provisioning.
  A first pull of a 3 GB GPU image measured 101,176 ms; once Modal has it cached,
  187 ms. Pre-warm before opening a GPU lane publicly.
...
- **Sandbox egress is filtered** (pypi and huggingface reachable, CloudFront reset), and
  `os.cpu_count()` reports the host's cores, not the lane's quota.
- **The `gpu-a100-80` cost basis is derived, not measured** — Modal's billing API is
  Team-tier only, so it is arithmetic on published rates and is marked
  `_costVerified: false`.
